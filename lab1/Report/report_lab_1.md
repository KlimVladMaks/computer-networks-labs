...
После этого пробуем занести в БД-кластер новые данные:

![](./assets/013.png)

И снова запускаем `pg-master`:
...
Также проверяем, что данные, загруженные при выключенном `pg-master` через `pg-slave` (который при выключенном `pg-master` стал лидером кластера), снова доступны через `pg-master`, что говорит об успешной синхронизации данных между узлами БД-кластера:
